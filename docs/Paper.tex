...
However it has been pointed out that there are no common tools for building such models \citep{grimm}. That is each of the $F(\vec{v}_{i,j})$ are individually designed and hand crafted by each of the investigators. Machine learning seems to be an excellent candidate for such a tool as it excels in finding functions $F(\vec{v}_{i,j})$ given that $\vec{v}_{i,j}$ and $c_i$ are known. One principle advantage of reducing the effort and time required to build a model would be the opportunity for more exploratory applications of individual based modeling. Specifically instead of having to make careful selections of appropriate information up front (given the effort in building models in the first place) investigators could build models using arbitrary sets of features and then learn the value of those features in making predictions by comparing the outcomes of the various models built using machine learning. 

The purpose of this paper then is to:

\begin{enumerate}
\item Present a guide as to one way in which to apply machine learning to individual based modeling where $\vec{v}_{i,j}$ and $c_i$ are available that is cost and computationally effective
\item Demonstrate the potential value of exploratory model building using an example application to Chinook salmon tagging data
\end{enumerate} 
